...
import { Check, History, RotateCcw, Users } from "lucide-react";
...
function Attendance() {
  const { students, subjects, attendance, saveAttendance } = useApp();
  ...
  useEffect(() => {
    ...
    setPresent(attendance[date]?.[subject] || Object.fromEntries(students.map((student) => [student.id, true])));
    setMessage("");
  }, [date, subject, attendance, students]);
  ...
  const history = useMemo(() => Object.entries(attendance).flatMap(([historyDate, records]) =>
    Object.entries(records).flatMap(([code, values]) => Object.entries(values).map(([id, isPresent]) => ({
      date: historyDate, code, student: students.find((item) => item.id === id), isPresent,
  ...
  const save = () => { saveAttendance(date, subject, present); setMessage("Attendance saved successfully."); };

  return <div className="dashboard">
    <div className="page-header"><div><h1>Attendance</h1><p>Record attendance and explore detailed history.</p></div><button className="primary-button" onClick={save}><Check size={18} /> Save Attendance</button></div>
